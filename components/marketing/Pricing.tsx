  ...
  return <div>
    <div className="mx-auto grid max-w-4xl gap-6 md:grid-cols-2">
      <article className="flex flex-col rounded-3xl border bg-surface p-7 sm:p-9">
        ...
        <ul className="my-7 flex-1 space-y-4">{["1 document verification check, once only", "Personalised document checklist", "Free preparation guides", "Sample interview report"].map(text => <li key={text} className="flex gap-3"><Check className="mt-1 h-4 w-4 shrink-0 text-primary" aria-hidden="true"/>{text}</li>)}<li className="flex gap-3 text-muted-foreground"><Minus className="mt-1 h-4 w-4 shrink-0" aria-hidden="true"/><span>No AI video interview</span></li></ul>
